...
function school_self_evaluation_posted_on() {
	...
	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'school-self-evaluation' ),
...
function school_self_evaluation_entry_footer() {
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'school-self-evaluation' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
		echo '</span>';
	}
